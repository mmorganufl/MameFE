...
from mame.RomSource import GenreRomSource, YearRomSource
...
class MainWindow(QtWidgets.QWidget):
    ...
    def showFullScreen(self):
        ...
        bannerHeight = int(height * BANNER_HEIGHT_PERCENTAGE)
        ...
        for i in range(0, numRows):
            ...
            row = TileRowWidget(self, 1, self._numColsToDisplay, count, self._romSource, self._headers[(i-1) % self._totalNumRows])
    ...
    def slideRow(self, moveDown):  
        ...
        if self._moveDown:
            ...
            row = TileRowWidget(self, 1, self._numColsToDisplay, count, self._romSource, self._headers[(self._selectedRow-1) % self._totalNumRows])
            ...
            row = TileRowWidget(self, 1, self._numColsToDisplay, count, self._romSource, self._headers[(self._selectedRow+3) % self._totalNumRows])
            row.setGeometry(self._newGeometry)            
            row.setParent(self)            
            row.initialize()           
